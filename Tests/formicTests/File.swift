...
func invokeBasicCommandOverCitadelSSHTEMP() async throws {
    ...
    //
    // swift test --filter FormicTests.invokeBasicCommandOverCitadelSSHTEMP
    //
    ...
    let absolutePathToTestingKey = bundle.path(
        forResource: "Formic_FormicTests.bundle/Contents/Resources/id_ed25519", ofType: nil)
    print("path: \(absolutePathToTestingKey)")

    try #require(absolutePathToTestingKey != nil)
    print(absolutePathToTestingKey)
    //    guard
    ...
    let host: Formic.Host? = try await withDependencies { dependencyValues in
        ...
        try await Formic.Host.resolve(
            hostname, sshPort: port, sshUser: username, sshIdentityFile: absolutePathToTestingKey,
