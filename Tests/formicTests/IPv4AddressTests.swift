...
func validAddressInit() async throws {
    ...
    let first = Host.IPv4Address(goodSample)
...
func failingAddressInit() async throws {
    ...
    let second = Host.IPv4Address(badSample1)
    #expect(second == nil)
}

@Test("parsing ipv4 address - localhost")
func localHostValidation() async throws {
    #expect(Host.IPv4Address.localhost.description == "127.0.0.1")
}

@Test("equatable ipv4 address")
func checkIPv4AddressEquatable() async throws {
    let first = Host.IPv4Address("127.0.0.1")
    let second = Host.IPv4Address("192.168.0.1")
    #expect(first != second)
}
