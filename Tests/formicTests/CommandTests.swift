...
func shellCommandDeclarationTest() async throws {
    let command = LocalProcess.shell("uname")
...
func verifyIdentifiableCommands() async throws {
    let command1 = LocalProcess.shell("uname")
    let command2 = LocalProcess.shell("uname")
...
func shellCommandFullDeclarationTest() async throws {
    let command = LocalProcess.shell(
        "ls", env: ["PATH": "/usr/bin"],
...
func copyCommandDeclarationTest() async throws {
    let command = LocalProcess.remoteCopy(from: "one", to: "two")
    #expect(command.retry == .none)
...
func copyCommandFullDeclarationTest() async throws {
    let command = LocalProcess.remoteCopy(
        from: "one", to: "two",
